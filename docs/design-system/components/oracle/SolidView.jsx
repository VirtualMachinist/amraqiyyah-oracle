...
function truncTetra() {
  ...
  for (let i = 0; i < 4; i++) for (let j = 0; j < 4; j++) {
    if (i === j) continue;
    out.push([
      TETRA[i][0] + (TETRA[j][0] - TETRA[i][0]) / 3,
      TETRA[i][1] + (TETRA[j][1] - TETRA[i][1]) / 3,
      TETRA[i][2] + (TETRA[j][2] - TETRA[i][2]) / 3,
    ]);
  }
  return out;
}
...
function hullFaces(pts) {
  const n = pts.length; const faces = []; const seen = new Set(); const EPS = 6e-3, TIGHT = 2.5e-3;
  const orderAround = (members, nv) => {
    const c = members.reduce((a, m) => [a[0] + pts[m][0], a[1] + pts[m][1], a[2] + pts[m][2]], [0, 0, 0]).map((x) => x / members.length);
    const u = normalize(sub(pts[members[0]], c)); const v = normalize(cross(nv, u));
    const sorted = members.slice().sort((a, b) =>
      Math.atan2(dot(sub(pts[a], c), v), dot(sub(pts[a], c), u)) - Math.atan2(dot(sub(pts[b], c), v), dot(sub(pts[b], c), u)));
    return { c, sorted };
  };
  for (let i = 0; i < n - 2; i++) for (let j = i + 1; j < n - 1; j++) for (let k = j + 1; k < n; k++) {
    let nv = cross(sub(pts[j], pts[i]), sub(pts[k], pts[i]));
    const ln = norm(nv); if (ln < 1e-9) continue;
    nv = scale(nv, 1 / ln); let d = dot(nv, pts[i]);
    if (d < 0) { nv = scale(nv, -1); d = -d; }
    if (d < 1e-6) continue;
    let ok = true;
    for (let m = 0; m < n; m++) { if (dot(nv, pts[m]) > d + EPS) { ok = false; break; } }
    if (!ok) continue;
    let members = [];
    ...
    for (let m = 0; m < o1.sorted.length; m++) {
      const a = pts[o1.sorted[m]], b = pts[o1.sorted[(m + 1) % o1.sorted.length]];
      nn = [nn[0] + (a[1] - b[1]) * (a[2] + b[2]), nn[1] + (a[2] - b[2]) * (a[0] + b[0]), nn[2] + (a[0] - b[0]) * (a[1] + b[1])];
    }
    nn = normalize(nn); if (dot(nn, o1.c) < 0) nn = scale(nn, -1);
    const dd = dot(nn, o1.c);
    // re-collect against the refitted plane with a tight tolerance
    let ok2 = true; const m2 = [];
    for (let m = 0; m < n; m++) { const t = dot(nn, pts[m]); if (t > dd + TIGHT) { ok2 = false; break; } if (t > dd - TIGHT) m2.push(m); }
    if (!ok2 || m2.length < 3) continue;
    const key = m2.join("-");
    if (seen.has(key)) continue; seen.add(key);
    const o2 = orderAround(m2, nn);
    faces.push({ normal: nn, d: dd, verts: o2.sorted, centroid: o2.c });
  }
  return faces;
}
...
function getSolid(name) {
  ...
  let verts;
  if (EMBED[name]) verts = EMBED[name];
  else if (GENERATED[name]) verts = cleanSet(GENERATED[name]());
  else if (DUAL_OF[name]) verts = polarDual(getSolid(DUAL_OF[name]).verts);
  else verts = TETRA;
  const r = Math.max(...verts.map(norm));
  verts = verts.map((v) => scale(v, 1 / r));
  const faces = hullFaces(verts);
  CACHE[name] = { verts, faces };
  return CACHE[name];
}

// ---------- render ----------
let GID = 0;
export function SolidView({ solid = "tetrahedron", size = 120, mirror = false, yaw = 0.55, pitch = -0.42, tone = "lapis", label, style }) {
  const { verts, faces } = React.useMemo(() => getSolid(solid), [solid]);
  const gid = React.useMemo(() => `sv${++GID}`, []);
  const cy0 = Math.cos(yaw), sy = Math.sin(yaw), cx0 = Math.cos(pitch), sx = Math.sin(pitch);
  const rot = (v) => {
    let x = v[0] * cy0 + v[2] * sy, z = -v[0] * sy + v[2] * cy0, y = v[1];
    const y2 = y * cx0 - z * sx, z2 = y * sx + z * cx0;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={style} aria-label={label || solid}>
      <defs>
      ...
      {drawn.filter((f) => !f.front).map((f, i) => (
        <path key={`b${i}`} d={f.path} fill="#1e3a8a" fillOpacity="0.10" stroke="#5d7fd0" strokeOpacity="0.22" strokeWidth={size * 0.004} />
      ))}
      {drawn.filter((f) => f.front).map((f, i) => (
        <path key={`f${i}`} d={f.path} fill="#33468c" fillOpacity={(0.12 + 0.22 * f.lit).toFixed(3)} stroke={edgeCopper} strokeOpacity="0.9" strokeWidth={size * 0.008} strokeLinejoin="round" />
      ))}
      {R.map((p, i) => (p[2] > 0.15 ? (
        <g key={`v${i}`}>
