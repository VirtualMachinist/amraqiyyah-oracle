...
**Sourcing doctrine (R-017; sharpened by R-055; widened by R-066 — the seats are LEADS, not fences: any stream may feed any register when the concept fits, and abundance outranks either/or — populate concept-fields from multiple sources as subtly-differentiated synonyms, the Arabic model of semantic richness):** English phonetic distillation is a last resort. New vocabulary is **concept-rooted from the source languages**: Egyptian is the primary neologism engine (a dead language with a vast attested lexicon — *ḥsb* "reckon" → Ḥ-S-B compute; *itrw* "river/Nile" → Y-T-R stream; *sš* "scribe" → S-Š write, the root of Seshat and of Sasha's own name); Muskogean leads land, life, kinship, and the everyday — and within it, **Mobilian Trade Jargon (Yama) is a ratified, freely-drawn source stream (R-064)**: the multi-ethnic contact lingua franca of the colonial Southeast that the Freedmen and the enslaved spoke alongside the Native nations, and one of the founding inspirations for Amraqiyyah itself. It is the *contact-language register* — the tongue of the crossing-over and the marketplace (H-K-L *hakolo* already carries it). Cited to the scholarly record (Crawford 1978; Kaufman 2017; Drechsel 1997) and to the sibling revitalization at **learnyama.com** (Mishasipi): where that project preserves what Mobilian *was*, Amraqiyyah carries it forward into what a language can newly *be*. Arabic/Hebrew lead the spiritual and legal — and per R-055, **Arabic holds the legal seat with the same systematic strength Greek holds for the technical**: the jurisprudential lexicon (ḥaqq, ʿadl, ḥukm, šarṭ, ʿaqd …) is the quarry when legal-institutional vocabulary lands; **Greek takes the technical-scientific-abstract seat** (diktyon → D-K-T network); AAVE remains the grammar stream and contributes genuinely vernacular culture. **Academic rigor is the standing standard (R-055)**: etymological and semantic precision govern the qamus especially, where scholarly completeness outranks phrasebook accessibility. Doc B's English-distilled tech roots (KMPT, TLVZ, STR, PST, TKS, NT, KD) are queued for re-derivation at D-5.
...
**Sandhi (R-026):** after consonant: plain (-u/-a/-i). After u: u+u→**ū**, u+a→**wa**, u+i→**wi**. After i: i+i→**ī**, i+a→**ya**, i+u→**yu**. After a: a+a→**ā**, a+u→**aw**, a+i→**ay**. **After ā (R-067, the dujā-class):** ā+u→**āw**, ā+a→**ā** (absorbed), ā+i→**āy** — the short-a row with length kept.
